    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="col-lg-4">
            <div class="title-action">
                ...
                if ($order_data['totalAmountPaidTillDate'] - $order_data['totalAmountReceivedTillDate'] == 0) {
                ?>
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#loanclose" <?php echo $order_data['is_finish'] != 0 ? 'disabled' : ''; ?>>
                        Close Loan
                    </button>
                <?php
                } else {
        ...
        <div class="col-lg-4">
            <div class="title-action">
                <a href="order_details?order_id=<?= $order_data['order_id'] ?>&page_name=print_form&print=true" target="_blank" class="btn btn-primary"><i class="fa fa-print"></i> Print
                    Form </a>
            </div>
        </div>
    </div>
            ...
            <div class="ibox-content p-xl">
                ...
                <div class="container">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm col-md-3 mt-3">
                            <b><?= strtoupper($order_data['loan_interest']) ?> % (Per Month) (प्रति महीने)</b>
                        </div>
                    </div>
                </div>
                <hr>
                        ...
                        <thead style="text-align: right;">
                            ...
                            <tr>
                                ...
                                <td style="background-color:skyblue; color:white; font-weight: bold;" class="text-dark ">
                                    <?= $order_data['totalprincipalAmountRest'] ?>
                                </td>
                            </tr>
                            <tr>
...
<script>
    ...
    const submit = () => {
        ...
        if (res.staus == "success") {
            $("#debitCreditModal").hide();
            window.location.reload();
        }
    }
    const loansubmit = () => {
        var date = $("#date").val()
        var loan_close_narration = $("#loan_close_narration").val()
        if (date == '') {
            alert("Please Select Date !")
            return false;
        }
        if (loan_close_narration == '') {
            alert("Please Enter Loan Close Narration !")
            return false;
        }
        var sendData = {
            "date": date,
            "loan_close_narration": loan_close_narration,
            "order_id": order_id,
        }
        const res = sendAjax("ajax/transactionUpdate", sendData);
        notify(res.status, res.msg);
    ...
    // $("#loancloseButton").on("click", function() {
    //     // location.reload(true);
    //     window.location.reload();
    // });
</script>
