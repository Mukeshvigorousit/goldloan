...
if (isset($_FILES['client_signature_pic']) && $_FILES['client_signature_pic']['error'] == 0) {
    ...
    if(isset($_FILES['aadhar_card_pic_front']) && $_FILES['aadhar_card_pic_front']['error']==0)
    {
        $name = $_FILES['aadhar_card_pic_front']['name'];
        $tmp_name = $_FILES['aadhar_card_pic_front']['tmp_name'];
        $size = $_FILES['aadhar_card_pic_front']['size'];
        $arr = explode(".",$name);
        $ext = end($arr);
        $new_name = time().rand(100000, 1000000).".".$ext;
        move_uploaded_file($tmp_name, "../img/item/item_full/".$new_name);
        $_POST['aadhar_card_pic_front']=$new_name;
    }


    if(isset($_FILES['aadhar_card_pic_back']) && $_FILES['aadhar_card_pic_back']['error']==0)
    {
        $name = $_FILES['aadhar_card_pic_back']['name'];
        $tmp_name = $_FILES['aadhar_card_pic_back']['tmp_name'];
        $size = $_FILES['aadhar_card_pic_back']['size'];
        $arr = explode(".",$name);
        $ext = end($arr);
        $new_name = time().rand(100000, 1000000).".".$ext;
        move_uploaded_file($tmp_name, "../img/item/item_full/".$new_name);
        $_POST['aadhar_card_pic_back']=$new_name;
    }
